...
import type { BotInstance, EnemyInstance, RunState, ShipSlot } from "../types/gameTypes";
...
export class RunScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    for (const slot of state.ship.slots) {
      ...
      const codeText = this.add.text(slot.x, slot.y, "", {
        fontFamily: "Verdana, sans-serif",
        fontSize: "16px",
        color: "#0b1217",
        align: "center",
  ...
  private drawPlayfield(state: RunState): void {
    ...
    this.graphics.arc(SHIP_CENTER.x, SHIP_CENTER.y, 146, Phaser.Math.DegToRad(-90), Phaser.Math.DegToRad(-90 + shieldRatio * 360), false);
    this.graphics.strokePath();

    const hullRatio = state.ship.hull / state.ship.maxHull;
    this.graphics.lineStyle(8, 0xe9d78d, 0.85);
    this.graphics.beginPath();
    this.graphics.arc(SHIP_CENTER.x, SHIP_CENTER.y, 160, Phaser.Math.DegToRad(-90), Phaser.Math.DegToRad(-90 + hullRatio * 360), false);
    this.graphics.strokePath();
  ...
  private drawSlots(state: RunState): void {
    ...
    for (const slot of state.ship.slots) {
      ...
      this.graphics.lineStyle(2, selected ? 0xb8efe9 : 0x5a7b8a, 0.9);
      ...
      if (!slot.moduleId) {
        codeText?.setText(state.phase === "planning" ? "+" : "").setColor("#9dc0d0");
        continue;
      }
  ...
  private updateTexts(state: RunState): void {
    this.phaseText.setText(`Cycle ${state.cycle} • ${state.phase.toUpperCase()} • Hull ${Math.round(state.ship.hull)}/${state.ship.maxHull} • Shield ${Math.round(state.ship.shield)}/${state.ship.maxShield}`);
    this.objectiveText.setText(`Moon Objective\nIntegrity ${Math.max(0, Math.round(state.simulation.objective.integrity))}/${state.simulation.objective.maxIntegrity}`);
    this.hintText.setText(
      state.phase === "planning"
        ? "Click a module in the sidebar, then click empty ship slots to place it. Select two adjacent filled slots to preview a merge."
        : "Execution is automatic. `Space` pauses instantly. Doctrine swaps are optional and cost 10% commitment each.",
    );
