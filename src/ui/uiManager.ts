...
export class UIManager {
  ...
  private renderMergePanel(
    ...
    let body = "Select any two or three placed modules to preview their merge.";
    let action = "";
    const botCapacity = getBotCapacity(state);
    const atBotCapacity = state.ship.bots.length >= botCapacity;
    ...
    if (selectedSlots.length >= 2 && selectedSlots.length <= 3 && selectedModules.length === selectedSlots.length) {
      ...
      if (preview.recipe) {
        if (atBotCapacity) {
          ...
          action = `<button class="ui-button secondary full-width" disabled>Support Bay Full</button>`;
        } else {
